...
class RTMClient extends BaseClient {
  ...
  receiveMsgCallback(message) {
    ...
    const text = this.decodeText(message);

    const user = new User(fromUserId, {
      sender: fromUserId,
      vchannel: message.vchannel_id,
      channel: message.channel_id || message.vchannel_id,
      type: message.type,
      name: fromUserId,
    });
  ...
  decodeText(message) {
    ...
    // Prepend hubot name for p2p message.
    if (message.type === MessageType.P2PMessage) {
  ...
  wsSend(message) {
    if (!this.wsConn) {
      this.robot.logger.error('websocket connection not open yet');
      return;
    }
